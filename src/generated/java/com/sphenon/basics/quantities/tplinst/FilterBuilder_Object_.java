...
public class FilterBuilder_Object_ implements GenericFilterBuilder<Object> {

    public FilterBuilder_Object_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Object.class)) : target_type);
    }
    ...
    protected GenericFilter<Object> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Object> filter = Factory_Filter_Object_.construct(context);

        if ((filter instanceof FilterByExpression_Object_) == false || operator == null || operator.matches("[=~]") == false) {
                               ...
                               ((FilterByExpression_Object_) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Object>> getter;
    protected Setter<GenericFilter<Object>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Object>> getter, Setter<GenericFilter<Object>> setter, Creator<GenericFilter<Object>> creator) {
        this.getter = getter;
        this.setter = setter;
