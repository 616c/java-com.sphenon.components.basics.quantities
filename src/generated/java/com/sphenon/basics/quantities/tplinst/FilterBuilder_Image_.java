...
public class FilterBuilder_Image_ implements GenericFilterBuilder<Image> {

    public FilterBuilder_Image_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Image.class)) : target_type);
    }
    ...
    protected GenericFilter<Image> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Image> filter = Factory_Filter_Image_.construct(context);

        if ((filter instanceof FilterByExpression_Image_) == false || operator == null || operator.matches("[=~]") == false) {
                               ...
                               ((FilterByExpression_Image_) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Image>> getter;
    protected Setter<GenericFilter<Image>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Image>> getter, Setter<GenericFilter<Image>> setter, Creator<GenericFilter<Image>> creator) {
        this.getter = getter;
        this.setter = setter;
